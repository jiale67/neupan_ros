...
> **后续进展**：本文描述的 `omni` 是 **2 自由度**模型（`u = (vx, vy)`，朝向不可控，
> 靠第 7 节那个外挂补偿环补角速度）。之后新增了 **3 自由度**的 `omni3`
> （`u = (vx, vy, w)`），把朝向交回规划器，V550 现在用的是它。
> 见 **[OMNI3.md](OMNI3.md)**。本文第 5/7/9 节里"`θ` 不可控"、"朝向只能靠外部
> 补偿环"这些结论**只对 `omni` 成立**，对 `omni3` 已不适用。
